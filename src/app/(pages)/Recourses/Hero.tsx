...
const Hero = () => {
  ...
  return (
    <>
      <section className='relative w-full pt-36  px-36 flex justify-center inset-x-0 m-auto secriontext-recurs'>
        <div className='flex flex-col m-16 justify-center'>
          ...
          transition={{ ease: "easeOut", duration: 0.3 }}
            ...
            <h1 className='text-[4.5rem] text-white title-rcurs'>{t('ALL RESOURCES')}</h1>
          </motion.div>
          <motion.div
          initial={{y: 100, opacity:0}}
          animate={{y: 0,opacity:1}}
          ...
          className='flex sp-container pt-3'>
            <p className='text-[1.2rem] text-white font-subtext text-rcurs'>{t('Find everything you need to After Effects, Davichi Resolve, Cap cut')}</p>
          </motion.div>
        </div>
      </section>
      <Herocontent />
